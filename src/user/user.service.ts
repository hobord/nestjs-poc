...
export class UserService {
  ...
  async update(userInput: UserUpdateInput): Promise<IUser> {
    ...
    if (user) {
      ...
      user.roles = await this.getUserRoleNames(user);
      if (userInput.roles) {
        const removeRoles = user.roles.filter(rolename => {
          return userInput.roles.indexOf(rolename) < 0;
        });
        await Promise.all(removeRoles.map(async rolename => {
          await this.removeUserRole(user, rolename);
        }));

        const addRoles = userInput.roles.filter(rolename => {
          return user.roles.indexOf(rolename) < 0;
        });
        await Promise.all(addRoles.map(async rolename => {
          await this.addUserRole(user, rolename);
        }));
  ...
  async updatePassword(user: IUser, newPassword: string): Promise<boolean> {
    const updateData = {
      ...user,
    } as UserInput;

    updateData.passwordHash = await this.getHash(newPassword);
    const updatedUser = await this.repository.update(user.id, updateData);
    if (updatedUser) {
      delete updatedUser.passwordHash;
      updatedUser.roles = await this.getUserRoleNames(user);
      return true;
    }
    return false;
  }
